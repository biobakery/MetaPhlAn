...
class Strainphlan:
    ...
    def get_matrix_for_sample(cls, sample_path, clade_markers, breadth_thres, trim_ends, polymorphism_perc,
        ...
        markers_matrix = {"sample_name": sample_name}
        markers_matrix.update({m: 0 for m in clade_markers})
        markers_matrix.update({marker.name: 1 for marker in cms.consensus_markers
...
def check_params(args):
    ...
    args.samples = Strainphlan.get_input_samples(args.samples)
    args.references = Strainphlan.get_input_samples(args.references)

    info(f'Will run with {len(args.samples)} samples and {len(args.references)} genomes')
