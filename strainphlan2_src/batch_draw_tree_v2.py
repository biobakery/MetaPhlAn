...
def read_params():
    ...
    p.add_argument('-l', '--leyend', type=str, default=None,
                   help="The leyend to draw the tree")
...
def check_params(args):
    if not args.clades:
        ...
        error('-a (or --metadata) must be specified', exit=True, 
            init_new_line=True)
        error('-l (or --leyend) must be specified', exit=True, 
            init_new_line=True)
...
"""
def batch_draw_tree(clades, output_dir, metadata, leyend):
    for c in clades:
        if check_clade(c):
            clade_dir = output_dir+c+"/results_base/"
            if os.path.isfile(clade_dir+"RAxML_bestTree.samples_as_markers.tre"):
                draw_tree(clade_dir+"RAxML_bestTree.samples_as_markers.tre", metadata, leyend)
...
if __name__ == "__main__":
    ...
    batch_draw_tree(args.clades, args.output_dir, args.metadata, args.leyend)
