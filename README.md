...
* The [MetaPhlAn](http://groups.google.com/forum/#!forum/metaphlan-users) and [StrainPhlAn](https://forum.biobakery.org/c/Microbial-community-profiling/StrainPhlAn/) Discourse forum.
...
For each sample, StrainPhlAn extracts the strain of a specific species by merging and concatenating all reads mapped against that species markers in the MetaPhlAn database.
...
If MetaPhlAn was installed through conda and you have the `mpa` activated, all the pre-requisites are satisfied.
...
This step will run MetaPhlAn to map all metagenomic samples against the MetaPhlAn marker database and produce the sam files (\*.sam.bz2).
Each sam file (in SAM format) corresponding to each sample contains the reads mapped against the marker database of MetaPhlAn.
...
do
    ...
    bn=$(basename ${f} | cut -d '.' -f 1)
     ../metaphlan.py --index mpa_v295_CHOCOPhlAn_201901 --input_type multifastq --nproc 10s -s sams/${bn}.sam.bz2 --bowtie2out sams/${bn}.bowtie2_out.bz2 -o ssams/${bn}.profile ${f}
done
```

After this step, you will have a folder "sams" containing the sam files (\*.sam.bz2) and other MetaPhlAn output files. 
...
Step 4. Extract the markers of *Bacteroides\_caccae* from MetaPhlAn database (to add its reference genome later):
