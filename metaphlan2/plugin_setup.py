...
plugin = Plugin(
    ...
    package='metaphlan2',
    citation_text=('Truong DT, Franzosa EA, Tickle TL, Scholz M, Weingart G, Pasolli E, Tett A, Huttenhower C, '
                   'Segata N. MetaPhlAn2 for enhanced metagenomic taxonomic profiling. Nature Methods. '
    ...
    description=('MetaPhlAn is a computational tool for profiling the composition of microbial communities '
                 '(Bacteria, Archaea, Eukaryotes, and Viruses) from metagenomic shotgun sequencing data with '
...
plugin.methods.register_function(
    ...
    parameter_descriptions={'nproc': ('The number of CPUs to use for parallelizing the mapping, '
                                      'default 1 (no parallelization)')},

    outputs=[('biom_table', FeatureTable[Frequency])],
    output_descriptions={'biom_table': ('Table relative abundances '
                                        'of the species found in the input')},
...
plugin.methods.register_function(
    ...
    parameters={'nproc': Int},
    parameter_descriptions={'nproc': 'The number of CPUs to use for parallelizing the mapping, default 1 '
                            '(no parallelization)'},

    outputs=[('biom_table', FeatureTable[Frequency])],
    output_descriptions={'biom_table': ('TAB-separated text file containing relative abundances of the species'
                                        ' found in the input')},

    name='MetaPhlAn2 taxonomic profiling',
    description=('MetaPhlAn is a computational tool for profiling the composition of '
                 'microbial communities (Bacteria, Archaea, Eukaryotes, and Viruses) '
                 'from metagenomic shotgun sequencing data with species level resolution')
)
